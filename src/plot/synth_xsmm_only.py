...
from tools import calc_FLOPS, load_benchmark_data, get_perf_xsmm_only, B_TARGET_PANEL_WIDTH

if len(sys.argv) < 7:
    print("expected 6 arguments: mat_dir n_runs matrix_size test_gimmik TIMESTAMP plot_dir")
    exit(1)
...
def plot(runs, mat_flops, x_term, trait, xlabel, title, save_as, \
    ...
    #         get_perf(runs, N_RUNS, trait, x_term, mat_flops, 0, TEST_GIMMIK)
    # else:
    x_values, ref_y_avg, ref_y_kernel = \
        get_perf_xsmm_only(runs, N_RUNS, trait, x_term, mat_flops, 0, TEST_GIMMIK)
